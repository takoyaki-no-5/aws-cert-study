...
- 全12資格とも同じ構成（見本: `study/clf/` / `study/saa/`）:
  - `exam.md` — **試験概要**（時間・問題数/形式・合格点・学習時間の一般vs今回。全資格共通フォーマット。見本: `saa/exam.md`）
...
- 立ち上げ日曜（受験2週間前、Pro は3〜4週間前）に日次を確定し、`days/` を埋める。新規追加時は `_templates/` をコピー（`exam.md` 含む）
