...
- **Discord** — 今日の**予定**通知のみ（スキル: `daily-plan-discord`、送信: `.cursor/hooks/discord-notify.py`、Secret: `discord_daily_bot`）。**進捗は送らない**。**現在 OFF**（`study/discord.off`・オン指示まで送らない）。平日枠は **帰宅後 2時間40分 + 電車 Anki 30分**
