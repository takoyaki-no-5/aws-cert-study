...
2. プロフィールに目標資格・期限・学習時間が欠けていれば、足りない分だけ聞いて `study/profile.md` に書く。平日枠は **2時間40分（160分）**。
3. 今日の予定の提示・Discord 送信が主目的ならスキル **`daily-plan-discord`** に従う（追加・スキップ・ズレ時は質問してから動く）。
4. 今日の `study/<資格>/days/YYYY-MM-DD.md` が無ければ `study/_templates/day.md` から生成（題材チェック・時間割のみ・短文・合計160分目安）。あればそれを提示。
